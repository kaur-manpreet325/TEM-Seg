...
class SegmentationModel(nn.Module):
    def __init__(self):
        ...
        self.unet = smp.UnetPlusPlus(  # Changed from smp.Unet to smp.UnetPlusPlus
...
for iteration in range(NUM_SELF_TRAINING_ITERATIONS):
    ...
    # Save pseudo-labels
    output_dir = os.path.join(mask_dir, f'UNet++_output_iteration_{iteration + 1}')
    ...
    if new_valid_loss > best_valid_loss:
        ...
        model.load_state_dict(torch.load('best_model_UNet++.pt'))
    else:
...
test_output_dir = os.path.join(mask_dir, 'UNet++_test_output')
