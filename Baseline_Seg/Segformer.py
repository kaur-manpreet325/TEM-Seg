...
def train_model(model, trainloader, validloader, epochs, patience):
    ...
    for epoch in range(epochs):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), 'best_model_Segformer.pt')
...
for iteration in range(NUM_SELF_TRAINING_ITERATIONS):
    ...
    # Save pseudo-labels
    output_dir = os.path.join(mask_dir, f'Segformer_output_iteration_{iteration + 1}')
    ...
    if new_valid_loss > best_valid_loss:
        ...
        model.load_state_dict(torch.load('best_model_Segformer.pt'))
    else:
...
test_output_dir = os.path.join(mask_dir, 'Segformer_test_output')
