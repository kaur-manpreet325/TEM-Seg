...
for iteration in range(NUM_SELF_TRAINING_ITERATIONS):
    ...
    # Save pseudo-labels
    output_dir = os.path.join(mask_dir, f'DeepLabV3+_output_iteration_{iteration + 1}')
    ...
    if new_valid_loss > best_valid_loss:
        ...
        model.load_state_dict(torch.load('best_model_DeepLabV3+.pt'))
    else:
...
test_output_dir = os.path.join(mask_dir, 'DeepLabV3+_test_output')
